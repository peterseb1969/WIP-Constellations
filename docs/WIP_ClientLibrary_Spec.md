...
WIP also ships an MCP (Model Context Protocol) server that exposes WIP’s capabilities as AI-native tools. The two interfaces serve different consumers at different stages:

| Interface | Consumer | Stage | Environment |
|---|---|---|---|
| **WIP MCP server** | The AI (Claude) during development | Phases 1–3: exploration, data model design, implementation | Claude Code / AI IDE |
| **@wip/client** | The running application | Phase 4: runtime, serving end users | Browser / Node.js |

Both apply the same bulk abstraction and error normalisation patterns over WIP’s HTTP APIs. They are complementary, not redundant:

- The MCP server is never referenced in application code.
- @wip/client is never used during the AI’s data model design phase.
- Data created via MCP tools is immediately available to apps using @wip/client, and vice versa — both talk to the same WIP instance.

The MCP server’s tool descriptions serve as the AI’s API documentation during development. The @wip/client’s TypeScript types serve as the developer’s API documentation at runtime. Same backend, same data, two interfaces optimised for their respective consumers.

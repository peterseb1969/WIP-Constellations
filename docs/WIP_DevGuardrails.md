...
During development, the React app runs on Vite's dev server (typically port 5173) and needs to reach WIP's API services (ports 8001–8005). This is a CORS problem in the browser. **Do not solve it by modifying WIP's Caddy configuration or by exploring the WIP codebase.** Vite has a built-in proxy that handles this cleanly.
...
The `@wip/client` must be configured to use **relative URLs** (no hostname), e.g., `/api/document-store/documents`. Vite's proxy intercepts these requests and forwards them to the correct WIP service port. The browser never sees the port differences; there are no CORS issues.

This means the app code is environment-agnostic: in development, Vite proxies the requests; in production, the gateway (Guide 1) routes them. The app doesn't change — only the infrastructure in front of it does.

**The constellation app never depends on WIP's internal Caddy configuration.** WIP's Caddy serves the WIP Console and handles OIDC redirects. Constellation apps have their own routing via the Vite proxy (development) or the gateway (production). These are separate concerns.
...
## Relationship to the WIP MCP Server

WIP also ships an MCP (Model Context Protocol) server that provides the same bulk abstraction and error normalisation, but for a different consumer: the AI during development time. The relationship is:

- **WIP MCP server** → used by the AI (Claude) during Phases 1–3 to explore WIP, create terminologies and templates, test documents, and validate data models. The AI calls MCP tools like `create_template` and `validate_document` instead of composing HTTP calls.
- **@wip/client / @wip/react** → used by the running application at runtime (Phase 4) to serve end users. Application code imports these packages and uses their typed APIs.

Both consume WIP's HTTP APIs internally and apply the same patterns: wrap single items into bulk requests, unwrap responses, extract per-item errors. They are not redundant — they serve different consumers at different stages of the development lifecycle. The MCP server is never used in application code; @wip/client is never used during the AI's data model design phase.
...
| **Phase 1: Exploratory**       | Use MCP tools to discover and inventory WIP      | Guide 1 (Gateway) — understand the deployment target                                                          |
| **Phase 2: Data Model**        | Design terminologies, templates, references; validate with MCP tools | Guide 4 (WIP Client) — understand the runtime interface (@wip/client and @wip/react)                |
| **Phase 3: Implementation**    | Use MCP tools to create terminologies, templates, test documents | Guide 3 (App Skeleton) — set up the project structure. Guide 7 (Testing) — write data layer tests.  |
| **Phase 4: Application Layer** | Build UI with @wip/client, integrate, deploy     | Guide 2 (UI Stack), Guide 5 (Data Entry), Guide 6 (Error Handling), Guide 7 (Testing) — all remaining guides. |

Note: Phases 1–3 use the WIP MCP server as the primary interface. Phase 4 switches to @wip/client and @wip/react for runtime code. See Guide 4 for the relationship between these two interfaces.

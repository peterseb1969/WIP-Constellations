...
const baseUrl = arg('url', 'https://localhost:8443')
const docUrl = arg('doc-url', '') // optional: hit document store directly
...
// Gateway client for template lookups (always needs full gateway)
const gwClient = createWipClient({
  baseUrl,
  auth: { type: 'api-key', key: apiKey },
})

// Document client — either direct to doc store or through gateway
const client = docUrl
  ? createWipClient({ baseUrl: docUrl, auth: { type: 'api-key', key: apiKey } })
  : gwClient

// Look up FIN_TRANSACTION template
async function getTemplateId(): Promise<{ id: string; version: number }> {
  const tmpl = await gwClient.templates.getTemplateByValue('FIN_TRANSACTION')
...
async function getAccountId(): Promise<string> {
  ...
  if (result.items.length > 0) {
    return result.items[0].document_id
  }
  throw new Error('No FIN_ACCOUNT documents found — create at least one account first')
}
...
function generateTransaction(
  ...
  return {
    ...
    data: {
      account: accountId,
      source_reference: `BENCH-FIXED-${index}`,
...
async function run() {
  ...
  console.log(`  Doc store:  ${docUrl || baseUrl} ${docUrl ? '(direct)' : '(via gateway)'}`)

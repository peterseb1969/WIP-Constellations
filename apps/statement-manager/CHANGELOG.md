...
- Added: Integration tests for Yuh PDF and Roche payslip parsers using pdfjs-dist legacy build against real files
- Added: Unit tests for all three parsers (UBS CSV, Yuh PDF, Roche payslip) -- 97 tests total
...
- Changed: Migrated Yuh and Roche parsers from `pdf-parse` to `pdfjs-dist` via shared `pdf-extract.ts` helper
...
- Added: Roche payslip parser with category mapping and summary extraction

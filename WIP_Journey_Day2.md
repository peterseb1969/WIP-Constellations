...
**2:20 — employer payslip parser committed.** All 4 payslips parse correctly, 18 line items each, subtotals filtered, payment amounts validate exactly. Extraction-first rule followed without prompting.
...
**3:00 — Content-based PDF detection.** Filename-based detection (`CP_REL-*` for Yuh, `PAYSLIP_RCH_*` for employer) was too brittle. Switched to detecting by PDF content (signature strings). Also fixed detached ArrayBuffer from double pdfjs-dist extraction.
...
**+0:00 — Parser tests.** 97 tests written across 5 files: UBS CSV unit tests (28), Yuh PDF unit (10) + integration (15), employer payslip unit (8) + integration (36). All passing. Committed.
...
- 3 import parsers collaboratively built: UBS CSV (28 tests), Yuh PDF (25 tests), employer payslip (44 tests)
...
| Commits | ~10 | 48 |
| Import parsers | 1 (UBS CSV, mapping issues found later) | 3 (UBS CSV, Yuh PDF, employer payslip, all collaboratively mapped) |
